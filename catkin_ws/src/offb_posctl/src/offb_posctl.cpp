...
int main(int argc, char **argv)//argc  argument count 传参个数，argument value
{
    ...
    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("mavros/state", 1,
                                                                 state_cb);
    ros::Subscriber plane_position_pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("/mhe_servo_pos", 1,
                                                                               plane_pos_cb);//pos+twist /mhe_pos
    ...
    ros::Subscriber plane_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>(
            "/mhe_servo_vel", 1, plane_vel_cb); //twist /mhe_vel
    ...
    // log输出文件初始化
    logfile.open("/home/mm/zzw/catkin_workspace/src/offb_posctl/log/log.csv", std::ios::out);
    ...
    // 读取PID参数
    std::string paraadr("/home/mm/zzw/catkin_workspace/src/offb_posctl/src/param");
...
//    string fileway = "/home/uav/lzy_ws/src/offb_posctl/data/screw.csv";
    string fileway = "/home/mm/zzw/catkin_workspace/src/offb_posctl/data/screw.csv";
...
//    ///set initial hover position and pose
    ...
    int count = 0;
    while (ros::ok()&&(count<15*controlfreq||abs(pose_drone_odom.pose.position.x-init_pos_x)>=0.1||true))
    {
        cout << "------------Hovering----------------" << endl;
        ros::spinOnce();
